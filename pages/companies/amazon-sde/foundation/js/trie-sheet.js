const completedQuestions =
    ...
    console.log(trieQuestions);

// Easy
const beginner = trieQuestions.filter(function(question){
    return question.difficulty === "Easy";
});

// Medium
const intermediate = trieQuestions.filter(function(question){
    return question.difficulty === "Medium";
});

// Hard
const advanced = trieQuestions.filter(function(question){
...
${
    ...
    ? `
        ...
        <a href="../problems.html?module=trie&id=${question.id}" class="solve-again">
            Solve Again →
        </a>
      `
    : `
        <a href="../problems.html?module=trie&id=${question.id}">
            Solve →
        </a>
      `
}
...
progressCount.textContent =
    `${completedQuestions.length} / ${trieQuestions.length} Questions`;
...
const completedQueueQuestions =
    completedQuestions.filter(function(id){
        ...
        return trieQuestions.some(function(question){
...
progressCount.textContent =
`${completedQueueQuestions.length} / ${trieQuestions.length} Questions`;

const progressPercentage =
(completedQueueQuestions.length / trieQuestions.length) * 100;
