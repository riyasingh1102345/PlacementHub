const completedQuestions =
    ...
    console.log(treesQuestions);

// Easy
const beginner = treesQuestions.filter(function(question){
    return question.difficulty === "Easy";
});

// Medium
const intermediate = treesQuestions.filter(function(question){
    return question.difficulty === "Medium";
});

// Hard
const advanced = treesQuestions.filter(function(question){
...
${
    ...
    ? `
        ...
        <a href="../problems.html?module=trees&id=${question.id}" class="solve-again">
            Solve Again →
        </a>
      `
    : `
        <a href="../problems.html?module=trees&id=${question.id}">
            Solve →
        </a>
      `
}
...
progressCount.textContent =
    `${completedQuestions.length} / ${treesQuestions.length} Questions`;
...
const completedQueueQuestions =
    completedQuestions.filter(function(id){
        ...
        return treesQuestions.some(function(question){
...
progressCount.textContent =
`${completedQueueQuestions.length} / ${treesQuestions.length} Questions`;

const progressPercentage =
(completedQueueQuestions.length / treesQuestions.length) * 100;
