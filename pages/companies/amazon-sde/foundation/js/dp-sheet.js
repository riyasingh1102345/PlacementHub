const completedQuestions =
    ...
    console.log(dpQuestions);

// Easy
const beginner = dpQuestions.filter(function(question){
    return question.difficulty === "Easy";
});

// Medium
const intermediate = dpQuestions.filter(function(question){
    return question.difficulty === "Medium";
});

// Hard
const advanced = dpQuestions.filter(function(question){
...
${
    ...
    ? `
        ...
        <a href="../problems.html?module=dp&id=${question.id}" class="solve-again">
            Solve Again →
        </a>
      `
    : `
        <a href="../problems.html?module=dp&id=${question.id}">
            Solve →
        </a>
      `
}
...
progressCount.textContent =
    `${completedQuestions.length} / ${dpQuestions.length} Questions`;
...
const completedQueueQuestions =
    completedQuestions.filter(function(id){
        ...
        return dpQuestions.some(function(question){
...
progressCount.textContent =
`${completedQueueQuestions.length} / ${dpQuestions.length} Questions`;

const progressPercentage =
(completedQueueQuestions.length / dpQuestions.length) * 100;
