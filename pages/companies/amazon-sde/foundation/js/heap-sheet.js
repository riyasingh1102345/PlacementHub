const completedQuestions =
    ...
    console.log(heapQuestions);

// Easy
const beginner = heapQuestions.filter(function(question){
    return question.difficulty === "Easy";
});

// Medium
const intermediate = heapQuestions.filter(function(question){
    return question.difficulty === "Medium";
});

// Hard
const advanced = heapQuestions.filter(function(question){
...
${
    ...
    ? `
        ...
        <a href="../problems.html?module=heap&id=${question.id}" class="solve-again">
            Solve Again →
        </a>
      `
    : `
        <a href="../problems.html?module=heap&id=${question.id}">
            Solve →
        </a>
      `
}
...
progressCount.textContent =
    `${completedQuestions.length} / ${heapQuestions.length} Questions`;
...
const completedQueueQuestions =
    completedQuestions.filter(function(id){
        ...
        return heapQuestions.some(function(question){
...
progressCount.textContent =
`${completedQueueQuestions.length} / ${heapQuestions.length} Questions`;

const progressPercentage =
(completedQueueQuestions.length / heapQuestions.length) * 100;
